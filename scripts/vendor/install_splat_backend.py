...
# This installer ships in the GitHub Releases bundle, NOT in the custom-node package.
# Locate an installed ComfyUI-SplatKit folder: installer.bat sets SPLATKIT_PACK_ROOT to the
# folder it was dropped into; --pack-root <folder> overrides it; and if this file still sits
# in the pack's tools/ dir, its own location is the fallback.
def _clean(p):
    # Tolerate a stray quote/space a caller's shell may append (e.g. a trailing backslash
    # inside quotes on Windows) so the path still resolves.
    return Path(str(p).strip().strip('"').strip()).resolve()

def _pack_root():
    args = sys.argv[1:]
    for i, a in enumerate(args):
        if a == "--pack-root" and i + 1 < len(args):
            return _clean(args[i + 1])
    env = os.environ.get("SPLATKIT_PACK_ROOT")
    if env:
        return _clean(env)
    return Path(__file__).resolve().parents[1]

PACK_ROOT = _pack_root()
if not (PACK_ROOT / "core" / "splatting" / "runtime.py").is_file():
    raise SystemExit(f"ComfyUI-SplatKit not found at {PACK_ROOT}. Pass --pack-root <folder> "
                     "or run installer.bat from inside the custom node's folder.")
...
# The gsplat wheel (URL, filename, SHA-256) lives with this installer -- it used to be in
# core/splatting/runtime.py, but the pack must not reference a binary download. Verified
# against WHEEL_SHA256 after download.
...
def can_migrate(facts, contract):
    ...
    if any(old.get(key) != contract[key] for key in ("python", "torch", "torchvision", "cuda", "gsplat")):
        return False
    wheel = runtime.TOOLS / WHEEL_NAME
    if not wheel.is_file() or sha256(wheel) != WHEEL_SHA256:
        return False
    script = """
...
def install(rebuild=False, keep_cache=False):
    ...
    wheel = download(WHEEL_URL, runtime.TOOLS / WHEEL_NAME, WHEEL_SHA256)
    uv = runtime.TOOLS / "uv.exe"
